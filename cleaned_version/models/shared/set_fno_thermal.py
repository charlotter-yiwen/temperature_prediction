...
class SetFNOModel(nn.Module):
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        return self.decoder(cond)               # (B, 1, H, W)
...
def load_count_sweep_data(params_path, temps_path, max_components=5, d_per_comp=3,
                          test_ratio=0.2, val_ratio=0.1, split_seed=42,
                          board_size=100.0, physics_norm=False, T_ambient=25.0):
    """Load count-sweep data (variable component counts, NaN-padded params).

    Params are normalised by physical range (not StandardScaler) so that
    zero-padded absent components remain exactly 0.

    When *physics_norm* is True the temperature field is first normalised
    to a thermal-impedance map:  T_norm = (T - T_ambient) / total_power.
    A StandardScaler is then fitted on these normalised fields.
    The raw test temperatures are always returned for evaluation.

    Returns
    -------
    p_tr, t_tr_s, p_val, t_val_s, p_te, t_te_raw, scaler_y, grid_size, norm_info
    """
    ...
    assert grid_size * grid_size == grid_total, f"Cannot infer square grid from {grid_total}"

    # (N, max_comp*d_per_comp) → (N, max_comp, d_per_comp)
    params_3d = params_raw.reshape(n_samples, max_components, d_per_comp).copy()

    # validity mask (True where component present)
    valid_mask = ~np.isnan(params_3d[:, :, 0])          # (N, max_comp)
    comp_counts = valid_mask.sum(axis=1)                 # per-sample

    # per-sample total power (before NaN→0)
    total_power = np.nansum(params_3d[:, :, 2], axis=1)  # (N,)
    total_power = np.maximum(total_power, 0.1)            # avoid div-by-zero

    # normalisation factors from valid entries
    valid_powers = params_3d[:, :, 2][valid_mask]
    max_power = float(valid_powers.max()) if valid_powers.size else 1.0
    ...
    # physics normalisation: thermal impedance  (°C / W)
    if physics_norm:
        temps_2d = (temps_2d_raw - T_ambient) / total_power[:, None, None]
        print(f"[Physics norm] T_norm range: [{temps_2d.min():.3f}, {temps_2d.max():.3f}] °C/W", flush=True)
    else:
        temps_2d = temps_2d_raw.copy()
    ...
    train_idx, test_idx = train_test_split(
        ...
        random_state=split_seed, stratify=comp_counts)

    p_train    = params_3d[train_idx]
    t_train    = temps_2d[train_idx]        # physics-normed (or raw)
    p_test     = params_3d[test_idx]
    t_test_raw = temps_2d_raw[test_idx]     # ALWAYS raw °C for evaluation
    tp_test    = total_power[test_idx]       # per-sample total power

    norm_info["total_power_test"] = tp_test

    # scale (possibly physics-normalised) temperatures
    scaler_y = StandardScaler()
    t_tr_flat = t_train.reshape(len(train_idx), -1)
    t_tr_s = scaler_y.fit_transform(t_tr_flat).reshape(-1, grid_size, grid_size).astype(np.float32)

    # validation split from training set
    if val_ratio > 0 and len(p_train) > 4:
        train_counts = comp_counts[train_idx]
        try:
            p_train, p_val, t_tr_s, t_val_s = train_test_split(
                p_train, t_tr_s, test_size=val_ratio,
                random_state=split_seed, stratify=train_counts)
        except ValueError:
            p_train, p_val, t_tr_s, t_val_s = train_test_split(
                p_train, t_tr_s, test_size=val_ratio,
                random_state=split_seed)
    else:
        p_val, t_val_s = None, None

    return (p_train.astype(np.float32), t_tr_s,
            p_val.astype(np.float32) if p_val is not None else None, t_val_s,
            p_test.astype(np.float32), t_test_raw,
            scaler_y, grid_size, norm_info)


def prepare_gen_test_data(params_path, temps_path, d_per_comp, norm_info, grid_size):
    """Prepare generalization-test data (unseen component counts).

    Uses the same normalisation factors that were computed during training.
    Returns raw temps and per-sample total_power for physics-aware evaluation.
    """
    params_raw = np.load(params_path).astype(np.float32)
    temps_raw  = np.load(temps_path).astype(np.float32)

    n = params_raw.shape[0]
    n_comp = params_raw.shape[1] // d_per_comp

    params_3d = params_raw.reshape(n, n_comp, d_per_comp).copy()
    ...
    temps_2d = temps_raw.reshape(n, grid_size, grid_size)  # raw °C
...
def inverse_transform_temps(preds_scaled, scaler_y, total_power=None, T_ambient=25.0):
    """Inverse-transform model predictions back to raw temperature (°C).

    When total_power is given (physics normalisation was used):
        T_raw = scaler_y.inverse(pred) * total_power + T_ambient
    Otherwise:
        T_raw = scaler_y.inverse(pred)
    """
    shape = preds_scaled.shape
    flat  = preds_scaled.reshape(len(preds_scaled), -1)
    out   = scaler_y.inverse_transform(flat).reshape(shape)
    if total_power is not None:
        out = out * total_power.reshape(-1, 1, 1) + T_ambient
    return out


def plot_thermal_comparisons(preds, true, scaler_y, out_dir, n_samples=6,
                             total_power=None, T_ambient=25.0):
    """For each of the first n_samples test cases: GT | Pred | |Error|."""
    n = min(n_samples, len(preds))
    for i in range(n):
        h, w = preds[i].shape[-2], preds[i].shape[-1]
        tp_i = total_power[i:i+1] if total_power is not None else None
        pred_img = inverse_transform_temps(
            preds[i:i+1], scaler_y, total_power=tp_i, T_ambient=T_ambient
        )[0]
        true_img = true[i].reshape(h, w) if true[i].ndim == 1 else true[i]
...
def main():
    ...
    # Data — legacy separate-file mode
    parser.add_argument("--train-params", default=None)
    parser.add_argument("--train-temps",  default=None)
    parser.add_argument("--test-params",  default=None)
    parser.add_argument("--test-temps",   default=None)

    # Data — count-sweep mode (single params / temps file, auto split)
    parser.add_argument("--count-sweep-params", default=None,
                        help="Path to params_count_sweep.npy")
    parser.add_argument("--count-sweep-temps", default=None,
                        help="Path to temps_count_sweep.npy")
    parser.add_argument("--test-ratio", type=float, default=0.2,
                        help="Fraction of count-sweep data held out for testing")

    # Generalization evaluation (unseen component counts)
    parser.add_argument("--gen-test-params", default=None,
                        help="Path to generalization-test params .npy (e.g. counts 6-8)")
    parser.add_argument("--gen-test-temps", default=None,
                        help="Path to generalization-test temps .npy")

    # Component layout  (n_comp × d_per_comp must equal params feature dim)
    parser.add_argument("--n-components", type=int, default=5,
                        help="Max components in training data")
    parser.add_argument("--d-per-comp",   type=int, default=3,
                        help="Feature dimension per component (3 = x_center, y_center, power)")
    ...
    parser.add_argument("--t-ambient", type=float, default=25.0,
                        help="Ambient temperature for physics normalization (°C)")
    ...
    for run_idx in range(args.n_runs):
        ...
        if args.count_sweep_params:
            p_tr, t_tr, p_val, t_val, p_te, t_te_raw, scaler_y, grid_size, norm_info = \
            ...
            p_tr, t_tr, p_val, t_val, p_te, t_te_raw, scaler_y = load_data(
                ...
                split_seed=split_seed)
            grid_size = int(round(np.sqrt(t_te_raw.shape[-1]))) if t_te_raw.ndim == 2 else t_te_raw.shape[-1]
            norm_info = None
        ...
        # serialise norm_info without numpy arrays (total_power_test is large)
        norm_info_save = {k: v for k, v in norm_info.items() if k != "total_power_test"} if norm_info else None
        ckpt = {
            ...
            "grid_size":       grid_size,
            "norm_info":       norm_info_save,
        }
        torch.save(ckpt, model_out_path)
        print(f"Model saved -> {model_out_path}", flush=True)

        # ── Physics-aware inverse helpers ──
        tp_test = norm_info.get("total_power_test") if norm_info else None
        T_amb   = norm_info.get("T_ambient", 25.0) if norm_info else 25.0
        use_phys = norm_info.get("physics_norm", False) if norm_info else False

        preds_scaled = predict_all(model, p_te, device, args.batch_size)
        preds_inv = inverse_transform_temps(
            preds_scaled, scaler_y,
            total_power=tp_test if use_phys else None,
            T_ambient=T_amb)
        t_te_grid = t_te_raw.reshape(-1, grid_size, grid_size) if t_te_raw.ndim == 2 else t_te_raw
        ...
        plot_thermal_comparisons(preds_scaled, t_te_grid, scaler_y, run_dir, n_samples=args.n_vis,
                                 total_power=tp_test if use_phys else None, T_ambient=T_amb)
        ...
        # ── Generalization evaluation (unseen component counts) ──
        gen_info = None
        if norm_info and args.gen_test_params and args.gen_test_temps:
            print("\n── Generalization evaluation ──", flush=True)
            gen_params, gen_temps, gen_comp_counts, gen_tp = prepare_gen_test_data(
                args.gen_test_params, args.gen_test_temps,
                d_per_comp=args.d_per_comp,
                norm_info=norm_info, grid_size=grid_size)

            gen_preds = predict_all(model, gen_params, device, args.batch_size)
            gen_preds_inv = inverse_transform_temps(
                gen_preds, scaler_y,
                total_power=gen_tp if use_phys else None,
                T_ambient=T_amb)

            gen_r2, gen_r2_avg = compute_r2(gen_preds_inv, gen_temps)
            print(f"Generalization Avg R²: {gen_r2_avg:.4f}", flush=True)

            # per-component-count breakdown
            gen_info = {"overall_r2": float(gen_r2_avg), "per_count": {}}
            for k in sorted(set(gen_comp_counts)):
                mask_k = gen_comp_counts == k
                r2_k = gen_r2[mask_k]
                avg_k = float(np.nanmean(r2_k[np.isfinite(r2_k)])) if np.isfinite(r2_k).any() else float('nan')
                print(f"  count={k}: n={mask_k.sum()}, Avg R²={avg_k:.4f}", flush=True)
                gen_info["per_count"][int(k)] = {"n": int(mask_k.sum()), "r2_avg": avg_k}

            # visualise a few generalization samples
            gen_dir = os.path.join(run_dir, "generalization")
            os.makedirs(gen_dir, exist_ok=True)
            plot_thermal_comparisons(gen_preds, gen_temps, scaler_y, gen_dir,
                                     n_samples=min(args.n_vis, len(gen_preds)),
                                     total_power=gen_tp if use_phys else None, T_ambient=T_amb)
            plot_r2_bar(gen_r2, gen_dir)
