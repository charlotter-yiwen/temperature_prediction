...
def generate_component_sweep_dataset(components_mm,
    ...
    Also save:
      - params_sweep.npy : [component_count, move_distance_mm]
    ...
    for comp_idx in range(n_comp):
        ...
        while generated < positions_per_component and attempts < positions_per_component * max_attempts_per_position:
            ...
            params_rows.append([float(n_comp), float(move_distance)])
            ...
            manifest.append({
                ...
                'params': [float(n_comp), float(move_distance)],
                'json_path': json_path,
                'png_path': png_path
            })
...
def generate_count_sweep_dataset(components_mm,
    ...
    Generate thermal maps by component COUNT:
      1 component -> 40 maps
      2 components -> 40 maps
      ...
      max_components components -> 40 maps
    ...
    if max_components < 1:
        raise ValueError("Need at least 1 component for count sweep")

    params_rows = []
    temps_rows = []
    manifest = []

    def build_xy_param_vector(shifted_components, max_comp_count):
        vec = np.full((max_comp_count * 2,), np.nan, dtype=np.float32)
        for idx, comp in enumerate(shifted_components):
            cx = 0.5 * (comp['x_min'] + comp['x_max'])
            cy = 0.5 * (comp['y_min'] + comp['y_max'])
            vec[2 * idx] = float(cx)
            vec[2 * idx + 1] = float(cy)
        return vec
    ...
    for k in range(1, max_components + 1):
        ...
        for idx, shifts in enumerate(variants[:positions_per_count], start=1):
            ...
            shift_tag = build_shift_filename(shifts)
            name = f"count{k}_idx{idx:02d}_{shift_tag}"
            ...
            xy_param = build_xy_param_vector(shifted, max_components)
            params_rows.append(xy_param)
            temps_rows.append(T.flatten(order='C'))
            ...
            manifest.append({
                ...
                'shifts': [list(pair) for pair in shifts],
                'xy_positions_mm': xy_positions,
                'params_schema': [f'x{i+1}' if j % 2 == 0 else f'y{i+1}'
                                  for i in range(max_components)
                                  for j in range(2)],
                'params': xy_param.tolist(),
                'json_path': json_path,
                'png_path': png_path
            })
    ...
    schema = {
        'description': 'Per-sample parameter vector is component center XY in mm, padded with NaN for missing components.',
        'max_components': int(max_components),
        'feature_order': [f'x{i+1}' if j % 2 == 0 else f'y{i+1}'
                          for i in range(max_components)
                          for j in range(2)],
...
def main():
    ...
    if args.components:
        ...
        # Default layout with 5 components
        components_mm = [
    ...
    if args.count_sweep:
        ...
        print(
            f"Generating count sweep: 1..{args.max_components}, "
            f"positions_per_count={args.positions_per_count}, "
            f"target_samples={args.max_components * args.positions_per_count}"
        )
        ...
        if len(components_mm) < 5:
            raise ValueError("component-sweep mode requires at least 5 components")
        print(
