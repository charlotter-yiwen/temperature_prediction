...
foreach ($exp in $experiments) {
    ...
    Write-Host "  Command: python cgan_cnn.py --epochs $SweepEpochs $($exp.args)"

    $argList = ("--epochs $SweepEpochs " + $exp.args) -split '\s+' | Where-Object { $_ -ne '' }
    $output = python cgan_cnn.py $argList 2>&1 | Out-String
...
Write-Host "  Command: python cgan_cnn.py --epochs $SweepEpochs $($best.Args)" -ForegroundColor Green
Write-Host "  Avg R2 : $($best.AvgR2)" -ForegroundColor Green

# Write sweep results to log
$logPath = Join-Path $PSScriptRoot "training_log.md"
...
python cgan_cnn.py $finalArgList
