...
=======================
用 thermal_prediction.py 生成 6 组件随机位置的温度数据。
每个样本的 6 个组件位置随机生成（不重叠）。
"""
import os
import sys
import json
...
POWERS = [2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5]  # 8组件全部2.5W
NUM_SAMPLES = 10
...
def random_component_positions(n=6, board_mm=100.0, comp_size_mm=8.0, powers=None, seed=None):
    if powers is None:
        powers = [2.0] * n
    if seed is not None:
        np.random.seed(seed)
...
def main():
    ...
    print(f"Generating {NUM_SAMPLES} random 6-component samples...")
    ...
    for i in range(NUM_SAMPLES):
        ...
        fname = build_filename(comps, n_comp=8)
