...
=======================
用 thermal_prediction.py 生成 9 组件随机位置的温度数据。
前8个组件功率2.5W，第九个组件功率10W。
每个样本的 9 个组件位置随机生成（不重叠）。
"""
import os
import sys
import json
...
# 前8个组件2.5W，第9个组件10W
POWERS = [2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 2.5, 10.0]
NUM_SAMPLES = 10
...
def random_component_positions(n=9, board_mm=100.0, comp_size_mm=8.0, powers=None, seed=None):
    if powers is None:
        powers = [2.0] * n
    if seed is not None:
        np.random.seed(seed)
...
def main():
    ...
    print(f"Generating {NUM_SAMPLES} random 9-component samples...")
    print(f"Powers: C1-C8 = 2.5W, C9 = 10W, Total = {sum(POWERS)}W")
    ...
    for i in range(NUM_SAMPLES):
        ...
        fname = build_filename(comps, n_comp=9)
