"""
predict_8comp.py
用 best 模型预测 8 组件泛化（功率全2.5W，总20W）
"""
...
def parse_gen_filename_8comp(filename):
    """从 count8 文件名解析 8 组件位置，功率全2.5W."""
    ...
    for i in range(0, len(nums) - 1, 2):
        x = int(nums[i])
        y = int(nums[i + 1])
        positions.append([x, y, 2.5])  # 8组件全2.5W
...
def load_gen_json(json_path):
    ...
    n_comp, positions = parse_gen_filename_8comp(fname)
    max_comp = 8
...
def main():
    ...
    # 加载 8 组件数据
    json_files = sorted([f for f in os.listdir(GEN_DIR) if f.startswith("count8") and f.endswith(".json")])
    print(f"Found {len(json_files)} count8 JSON files")
    ...
    print(f"8-comp gen_test: params {gen_params.shape}, temps {gen_temps.shape}")
    ...
    print(f"\n{'='*60}")
    print(f"  8-Component Generalization (Total Power = 20.0W)")
    print(f"{'='*60}")
