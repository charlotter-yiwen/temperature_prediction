    ...
    while iteration < max_iterations and max_diff > tolerance:
        ...
        if iteration % 500 == 0:
            ...
            if current_max_temp > ambient_temp + 200:
                ...
                # Save base result with zero shifts: filename like 0,0,0,0.json and matching .png
                zero_flat = []
                for _ in components_mm:
                    zero_flat.extend(['0', '0'])
                zero_name = ','.join(zero_flat) if zero_flat else '0'
                zero_json_path = os.path.join(args.output, f"{zero_name}.json")
                if not os.path.exists(args.output):
                    os.makedirs(args.output)
                with open(zero_json_path, 'w', encoding='utf-8') as f:
                    json.dump(temp_data, f, indent=2)
                # save png
                zero_png_path = os.path.join(args.output, f"{zero_name}.png")
                fig = plot_temperature(T, components, (100.0, 100.0), save_path=zero_png_path, show_plot=False)
                plt.close(fig)
