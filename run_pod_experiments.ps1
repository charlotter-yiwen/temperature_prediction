...
foreach ($exp in $experiments) {
    ...
    Write-Host "  Command: python pod.py $($exp.args)"

    if ($exp.args -eq "") {
        $output = python pod.py 2>&1 | Out-String
    } else {
        $output = python pod.py ($exp.args -split ' ') 2>&1 | Out-String
    }
...
Write-Host "  Command: python pod.py $($best.Args)" -ForegroundColor Green
