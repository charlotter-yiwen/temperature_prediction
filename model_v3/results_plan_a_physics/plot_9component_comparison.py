"""
plot_9component_comparison.py
Generate GT vs Prediction comparison heatmaps for 9 component generalization data.
"""
...
OUT_DIR   = os.path.join(MODEL_DIR, "gen_comparison", "9_Component_30W")
...
def main():
    ...
    n_comp = 9
    # 9 components at 30W total - each component roughly 3.3W
    powers = [3.3] * 9

    json_files = sorted([f for f in os.listdir(GEN_DIR)
                        if f.startswith(f"count{n_comp}") and f.endswith(".json")])

    print(f"Found {len(json_files)} 9-component samples")
    ...
    print(f"\n9-Component (30W): {n} samples, predicting...")
    ...
    for i in range(n):
        fname = json_files[i]
        r2 = r2_vals[i]
        title = f"9-Component (30W) Sample {i+1:02d}  R2={r2:.4f}"
        save_path = os.path.join(OUT_DIR, f"sample_{i+1:02d}.png")
        plot_comparison(preds_inv[i], temps_2d[i], title, save_path)

    # Summary
    print(f"\n  9-Component Overall R2: {np.nanmean(r2_vals):.4f}")
