...
$baseOut = Join-Path $workspace "thermal_analysis_output_sets"
...
foreach ($n in $VariantCounts) {
    ...
    & python "thermal_prediction.py" --output $outFolder --grid-size $GridSize --max-shift $MaxShift --variants $n 2>&1 | Out-Null

    # Step 2: Process and split into train/test npy
    Write-Host "  -> process_json_to_grid.py --split" -ForegroundColor DarkGray
    & python "process_json_to_grid.py" $outFolder --split --train-ratio $TrainRatio --xi $Xi --yi $Yi --subsample $Subsample 2>&1 | Out-Null
